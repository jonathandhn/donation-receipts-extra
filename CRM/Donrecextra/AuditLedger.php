    ...
    if (!CRM_Core_DAO::checkTableExists('civicrm_donrecextra_receipt_audit')) {
      if (function_exists('_donrecextra_civix_schema')) {
        try {
          ...
          // Fall back to direct DDL below if CiviMix schema helper is unavailable
        }
      }
    }
    ...
    if (!CRM_Core_DAO::checkTableExists('civicrm_donrecextra_receipt_audit')) {
      ...
      CRM_Core_DAO::executeQuery("
        CREATE TABLE IF NOT EXISTS `civicrm_donrecextra_receipt_event` (
          ...
          CONSTRAINT `FK_donrecextra_event_audit` FOREIGN KEY (`receipt_audit_id`) REFERENCES `civicrm_donrecextra_receipt_audit` (`id`) ON DELETE RESTRICT
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
      ");
      CRM_Core_DAO::executeQuery("
        CREATE TABLE IF NOT EXISTS `civicrm_donrecextra_receipt_item_audit` (
          ...
          CONSTRAINT `FK_donrecextra_item_audit` FOREIGN KEY (`receipt_audit_id`) REFERENCES `civicrm_donrecextra_receipt_audit` (`id`) ON DELETE RESTRICT
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
      ");
      CRM_Core_DAO::executeQuery("
